...
**AWS実装時は、事前学習の知識に頼らず、必ずMCPでAWSドキュメントを確認する**
...
詳細な実装方法は `aws-cloudformation-guidelines.md` の「ネストされたスタック実装」を参照。
...
4. **値の受け渡し**: パラメータ経由を優先（Export/Import は避ける）

#### 設計時の注意点

- **粒度**: 機能単位で適切に分割（細かすぎず、大きすぎず）
- **独立性**: 各子スタックは可能な限り独立して動作するよう設計
- **再利用性**: 汎用的な機能は再利用可能なモジュールとして設計
- **命名**: 機能が明確にわかる名前を付ける
...
2. **AWS ベストプラクティスの調査**
   ...
   - AWS Well-Architected Framework の推奨事項を確認
   - 各サービスの公式ドキュメントで推奨される監視メトリクスを確認

3. **必須監視項目の特定**
   - **要件・設計書に記載されている監視項目**：必ず実装
   - **AWS ベストプラクティスで必須とされる項目**：必ず実装
   - **優先度が低く、要件・設計書に記載されていない項目**：実装不要

##### 実装時の必須確認事項

- **CFN Lint 検証**
   - 実装後は必ず `cfn-lint` で検証（exit code 0 必須）

##### 監視項目追加の判断基準

**追加すべき場合：**
- ✅ 要件・設計書に明記されている
- ✅ AWS ベストプラクティスで必須とされている
- ✅ システムの健全性に直接影響する
