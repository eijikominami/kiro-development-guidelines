...
#### 例
```
申し訳ありません！調べてほしいとお願いされただけで、
更新してほしいとは言われていませんでした。元に戻します。

[変更を元に戻す作業を実行]

調査結果として、以下の情報をお伝えします：
[情報提示のみ]
```
...
**疑わしい場合は情報提供のみに留める。明示的な変更指示がない限り、ファイルの変更は行わない。**

### 提案・推奨時の重要ルール

**提案や推奨を行う際は、実際の変更は行わず、提案内容のみを提示する**

#### 適用場面
- 「〜が良いと思います」「〜を推奨します」などの提案時
- 「〜という名前にしとくのが無難？」などの質問への回答時
- 改善案や代替案を提示する時
- 命名規則やベストプラクティスを提案する時

#### 対応方法
1. **提案内容を明確に説明する**
2. **理由や根拠を示す**
3. **実際の変更は行わない**
4. **「変更しますか？」「実装しますか？」などの確認を求める**
...
## 推奨案
- VisualETLJobForGA4Ingestion （最も推奨）
- VisualETLJobGA4Ingestion
- GA4IngestionETLJob

この命名にしますか？それとも現在のままで良いですか？
```

#### 避けるべき行動
- ❌ 提案と同時に勝手に変更する
- ❌ 「良いと思うので変更します」と一方的に実行する
- ❌ 確認なしで「改善」として変更する

#### 正しい行動
- ✅ 提案内容を説明してから確認を求める
- ✅ 変更するかどうかはユーザーの判断に委ねる
- ✅ 明示的な変更指示があってから実行する
...
##### 1. スペーシングルール（可読性のため重要）
- **ルール**: 日本語テキストと英語単語・略語の間に必ずスペースを追加
- **例**:
